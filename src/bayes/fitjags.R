...
fitjags$.buildModel <- function(trainingData, modelname, nchains, iterAdapt, phen, colsToIndex)
{
    ...
    dataForJags   = c(dataForJags, countPerFactor, covmatPerFactor)
...
fitjags$.buildSamples <- function(jags, observeInfo, iterSample, trainingData, thin=1)
{
    vals = coda.samples(model=jags, 
			...
			n.iter =iterSample,
			thin=thin, quiet=!showTiming,progress.bar="none")[[1]] #single chain for now
    gc()
    if(showTiming)
    {
        print("done sampling")
